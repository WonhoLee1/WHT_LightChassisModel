...
def paraview_warp_vector(result_data: "WHTResultData",
                         transient_geometry: bool = True) -> Optional[str]:
    """
    ParaView 자동 Warp에 사용할 변위 벡터 필드명을 반환한다. 없으면 None.

    내보낸 형상이 '미변형'일 때만 Warp가 필요하다. transient_geometry=True 이고
    T>1 이면 익스포터가 변위를 점 좌표에 이미 구워 넣으므로(모달/동적 애니메이션)
    Warp를 적용하면 이중 변형이 된다 → None.
    """
    pre_deformed = bool(transient_geometry) and result_data.n_timesteps > 1
    if pre_deformed:
        return None
    for key in ("Displacement", "ModeShape", "Eigen"):
        if key in result_data.point_data:
            return key
    return None


def _write_paraview_warp_script(hdf_path: str, vector: str, scale: float,
                                temp_dir: str) -> str:
    """ParaView 시작 시 실행할 Warp By Vector 자동 적용 스크립트를 생성한다."""
    data_path = os.path.abspath(hdf_path).replace("\\", "/")
    script_path = os.path.join(temp_dir, "wht_paraview_autowarp.py")
    script = f'''# Auto-generated by WHT - applies Warp By Vector on load.
from paraview.simple import *
src = OpenDataFile(r"{data_path}")
UpdatePipeline()
try:
    arrays = list(src.PointData.keys())
except Exception:
    arrays = []
if "{vector}" in arrays:
    w = WarpByVector(Input=src)
    w.Vectors = ["POINTS", "{vector}"]
    w.ScaleFactor = {float(scale)}
    rep = Show(w)
    Hide(src)
else:
    rep = Show(src)
...
def launch_paraview(file_path: str, warp_vector: Optional[str] = None,
                    warp_scale: float = 1.0) -> bool:
    """
    ...
    warp_vector : 지정 시 ParaView 시작 스크립트를 생성해 해당 벡터(point data)로
                  Warp By Vector를 자동 적용한다. None이면 파일만 연다.
    warp_scale  : Warp 배율.
    """
    import subprocess
    import shutil
    ...
    # warp_vector 지정 시: 시작 스크립트로 Warp By Vector 자동 적용 (파일은 스크립트가 연다)
    launch_args = [paraview_bin, abs_path]
    if warp_vector:
        try:
            script_path = _write_paraview_warp_script(
                abs_path, warp_vector, warp_scale, os.path.dirname(abs_path))
            launch_args = [paraview_bin, f"--script={script_path}"]
            print(f" -> [ParaView Launcher] 자동 Warp By Vector 적용: '{warp_vector}' (scale={warp_scale})")
        except Exception as _se:
            print(f" -> [ParaView Launcher] Warp 스크립트 생성 실패({_se}). 파일만 엽니다.")
            launch_args = [paraview_bin, abs_path]
...
def visualize_in_paraview(result_data: "WHTResultData", temp_dir: Optional[str] = None) -> bool:
    ...
    # 정적(미변형) 결과는 Displacement로 자동 Warp, 모달/동적(이미 변형됨)은 생략
    warp_vec = paraview_warp_vector(result_data, transient_geometry=True)
    return launch_paraview(hdf_path, warp_vector=warp_vec)
