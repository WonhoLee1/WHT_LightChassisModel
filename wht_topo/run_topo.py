...
def extract_dynamic_snapshots(model, node_db, csv_path, t_start,
    ...
    print(f"\n [Dynamic] 동적 해석 수행 (CSV: {csv_path}, Start: {t_start}s)")
    ...
    if use_global_z:
        ...
        for i, lbl in enumerate(corner_labels):
            for j, ax in enumerate(['X', 'Y', 'Z']):
                traj_mm[:, i, j] = df[f"{lbl}_pos_{ax}"].to_numpy() * 1000.0
    else:
        corner_z, traj_mm = calculate_local_z_history(df, time_arr)
    ...
    if add_inertia:
        for nid in node_db:
            ...
            mass = m_diag[ix * 6 + 2]
            if mass > 0:
                lg_i = _InterpLoadGroup(node_ids=[nid], dof=2,
                                        time_arr=time_arr, disp_arr=-mass * a)
...
def run_industrial_topo(args):
    ...
    # 동적 스냅샷 추출 (--dynamic-opts 지정 시)
    dynamic_load_cases = None
    if getattr(args, 'dynamic_opts', None):
        opts = [s.strip() for s in args.dynamic_opts.split(',')]
        csv_path = opts[0]
        t_start = float(opts[1]) if len(opts) > 1 else 1.6
        add_inertia = getattr(args, 'add_inertia', False)
        use_global_z = getattr(args, 'use_global_z', False)
        dyn_grid = getattr(args, 'dyn_grid', 3)

        dyn_snaps = extract_dynamic_snapshots(
            model, node_db, csv_path, t_start,
            grid_n=dyn_grid, add_inertia=add_inertia,
            use_global_z=use_global_z,
        )
        # 정적 케이스 + 동적 스냅샷을 합쳐서 solver에 주입
        static_cases = load_manager.get_load_cases(
            mesh_size_z=args.mesh_size, weights=weights)
        dynamic_load_cases = static_cases + dyn_snaps
        print(f" -> [Dynamic] 정적 {len(static_cases)} + 동적 {len(dyn_snaps)} = "
              f"총 {len(dynamic_load_cases)}개 하중 케이스")
...
def main():
   ...
   10. 동적-토포 통합 최적화 (실측 충격 데이터 반영, 추천):
       python wht_topo/run_topo.py --dynamic-opts "wht_topo/sample_pos.csv, 1.6" --add-inertia

   11. 고해상도 동적 분석 (4x4 그리드로 최대 16개 스냅샷 추출):
       python wht_topo/run_topo.py --dynamic-opts "data.csv, 1.6" --dyn-grid 4 --add-inertia

   12. 글로벌 궤적 비교 분석 (시각적 확인용):
       python wht_topo/run_topo.py --dynamic-opts "data.csv, 1.6" --use-global-z

   13. 동적 + 대칭 + 연결 산업용 최종 설계:
    ...
    # ── 동적 하중 통합 최적화 ─────────────────────────────────────────────
    parser.add_argument("--dynamic-opts", type=str, default=None,
                        help="동적 스냅샷 추출: 'CSV경로,시작시간' e.g. 'sample.csv,1.6'")
    parser.add_argument("--add-inertia", action="store_true",
                        help="동적 해석 시 관성 하중(-ma) 포함 (추천)")
    parser.add_argument("--use-global-z", action="store_true",
                        help="로컬 변형 대신 글로벌 Z 궤적 직접 추종 (시각화용)")
    parser.add_argument("--dyn-grid", type=int, default=3,
                        help="동적 피크 탐색 그리드 분할 수 NxN (기본: 3)")
