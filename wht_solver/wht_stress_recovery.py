...
class ElementStressRecovery:
    ...
    def recover_quad4(wht_model, u_global_array, sorted_nids):
        ...
        if not eid_list:
            return out_stresses, out_strains
        ...
        a = norm_V1[:, 0] / 2.0
        b = np.linalg.norm(V2, axis=1) / 2.0
        ...
        eps_loc[:, 0, 0], eps_loc[:, 1, 1], eps_loc[:, 0, 1], eps_loc[:, 1, 0] = eps_xx, eps_yy, gamma_xy/2.0, gamma_xy/2.0
    ...
    def recover_tria3(wht_model, u_global_array, sorted_nids):
        ...
        # 3. CST Membrane + Bending 계산 (요소 윗면 z = +t/2)
        x2 = norm_V1[:, 0]
