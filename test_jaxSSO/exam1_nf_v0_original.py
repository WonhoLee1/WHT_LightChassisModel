...
def run_nf_analysis():
    ...
    width, length, height = 100.0, 100.0, 10.0
    mesh_size = 5.0
    node_db, elem_db = generate_shell_tray(width, length, height, mesh_size)
    ...
    for eid, nids in elem_db.items():
        if len(nids) == 4:
            ...
            for nid in nids:
                ...
                M_diag[nid*6+3 : nid*6+6] += mass_per_node * (mesh_size**2) / 12.0 + 1e-15 # small epsilon
                
    # 7. Solve Eigenvalues
    known_id = model.known_id
    unknown_id = model.unknown_id
    ...
    plotter.add_mesh(grid, style='wireframe', color='black', opacity=0.1, label='Undeformed')
    # Show Warped Mesh
    plotter.add_mesh(warped, scalars="Disp_Magnitude", cmap="jet", 
                     show_edges=True, edge_color="white", line_width=1,
                     smooth_shading=False) # Turn off smooth shading to see raw elements
    
    plotter.add_scalar_bar(title="Displacement")
