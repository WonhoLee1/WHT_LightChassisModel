...
def run():
    ...
    try:
        ...
        res_modal = solver.solve_modal(num_modes=20, method="sparse",
    ...
    for lc, _ in lcs:
        try:
            t0 = time.time()
            solver = WHTSolver(model)
            res_static = solver.solve_static(lc, solver_method="sparse")
            t_s = time.time() - t0
    ...
    out_path = os.path.join(os.path.dirname(__file__), "WHT_Scipy_results.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(out_dict, f, indent=2)
    print(f"[WHT_Scipy] 완료. {out_path}")
